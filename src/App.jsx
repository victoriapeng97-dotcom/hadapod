          ...
          <div className="fade-in" style={{ padding: "32px 24px", maxWidth: 960, margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20 }}>
              <div style={{ background: "rgba(255,255,255,0.7)", backdropFilter: "blur(16px)", border: "1px solid rgba(212,184,150,0.3)", borderRadius: 24, overflow: "hidden", boxShadow: "0 8px 40px rgba(160,110,80,0.1)" }}>
                {(routineSteps[routineTime] || DEFAULT_ROUTINE_STEPS).map((step, i) => {
                  ...
                  return (
                    <div key={step.id} draggable onDragStart={() => setDraggedStep(step.id)} onDragOver={(e) => e.preventDefault()}
                      onDrop={() => {
                        ...
                        const steps = [...(routineSteps[routineTime] || DEFAULT_ROUTINE_STEPS)];
                      ...
                      style={{ display: "flex", alignItems: "center", gap: 14, padding: "15px 20px", borderBottom: i < (routineSteps[routineTime] || DEFAULT_ROUTINE_STEPS).length - 1 ? "1px solid rgba(212,184,150,0.15)" : "none", background: done ? "rgba(200,135,122,0.04)" : "transparent", transition: "all 0.2s", cursor: "grab" }}>
                        ...
                        <input autoFocus value={editingStepName} onChange={e => setEditingStepName(e.target.value)}
                          onBlur={() => {
                            if (editingStepName.trim()) {
                              const updated = { ...routineSteps, [routineTime]: (routineSteps[routineTime] || DEFAULT_ROUTINE_STEPS).map(s => s.id === step.id ? { ...s, name: editingStepName.trim() } : s) };
                              setRoutineSteps(updated);
                              if (auth.currentUser) saveUserData(auth.currentUser.uid, { routineSteps: updated });
                            }
                      ...
                      <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                        <button onClick={() => { setEditingStepId(step.id); setEditingStepName(step.name); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#B8A090", padding: 4 }}>✏️</button>
                        ...
                        <button onClick={() => {
                          const updated = { ...routineSteps, [routineTime]: (routineSteps[routineTime] || DEFAULT_ROUTINE_STEPS).filter(s => s.id !== step.id) };
                          setRoutineSteps(updated);
                          if (auth.currentUser) saveUserData(auth.currentUser.uid, { routineSteps: updated });
                    ...
                    <input autoFocus value={newStepName} onChange={e => setNewStepName(e.target.value)} placeholder="Step name..."
                      onKeyDown={e => {
                        if (e.key === "Enter" && newStepName.trim()) {
                          ...
                          const updated = { ...routineSteps, [routineTime]: [...(routineSteps[routineTime] || DEFAULT_ROUTINE_STEPS), newStep] };
                          setRoutineSteps(updated);
                          if (auth.currentUser) saveUserData(auth.currentUser.uid, { routineSteps: updated });
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {["AM", "PM"].map(t => {
                  const steps = routineSteps[t] || DEFAULT_ROUTINE_STEPS;
            ...
            {showAddToRoutine && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setShowAddToRoutine(null)}>
                <div style={{ background: "#FBF7F2", borderRadius: "24px 24px 0 0", padding: "28px 24px", width: "100%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 700, color: "#2A2018", marginBottom: 6 }}>{showAddToRoutine.pickStep ? `Add ${showAddToRoutine.item?.name || "product"} to Routine` : `Add to ${showAddToRoutine.step?.name}`}</div>
                  <div style={{ fontFamily: "'Jost',sans-serif", fontSize: 13, color: "#A09080", marginBottom: 20 }}>{showAddToRoutine.pickStep ? "Choose which step to add it to" : "Choose a product from your collections"}</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 300, overflowY: "auto" }}>
                    {showAddToRoutine.pickStep ? (
                      ((routineSteps && routineSteps[showAddToRoutine.time]) || DEFAULT_ROUTINE_STEPS).map(step => (
                        <div key={step.id} onClick={() => {
                          const updated = { ...routine, [showAddToRoutine.time]: { ...routine[showAddToRoutine.time], [step.id]: showAddToRoutine.item } };
                          setRoutine(updated);
                          if (auth.currentUser) saveUserData(auth.currentUser.uid, { routine: updated });
                          setShowAddToRoutine(null);
                          showNotif(`Added to ${step.name} ✓`);
                          ...
                          <div style={{ fontFamily: "'Jost',sans-serif", fontSize: 13, fontWeight: 600, color: "#2A2018" }}>{step.name}</div>
                        </div>
                      ))
